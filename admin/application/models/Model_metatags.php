...
class Model_metatags extends CI_Model {
...
public function add()
		...
		$img=$fname=time().'image'.rand().'.jpg';	
			...
			$add=array(
				...
				'biography'=>$biography,
				'reaserchinterst'=>$reaserchinterst,
				'image'=>$img	
				);
			$res=$this->db->insert('metatags',$add);
...
public function singleView()
{
	...
	$query=$this->db->get_where('metatags',array('id'=>$id))->row();
	return $query;
}
...
public function update()
{
	...
	extract($_POST);
			...
			$ip=$_SERVER['REMOTE_ADDR'];
			if(@$nm!=""){
				$gt=$this->db->get_where('metatags',array('id'=>$id))->row();
			...
			$res=$this->db->update('metatags',$add, array('id'=>$id) );
			if($res==true)
			{
				$config['file_name']  = $fname;
				$image_path = realpath(APPPATH . '../assets/aboutpage/');
