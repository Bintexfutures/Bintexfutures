...
class Model_scientificprograms extends CI_Model {
...
public function add()
		...
		$img=$fname=time().'image'.rand().'.jpg';	
			...
			$res=$this->db->insert('scientific',$add);
...
public function singleView()
{
	...
	$query=$this->db->get_where('scientific',array('id'=>$id))->row();
	return $query;
}
...
public function update()
{
	...
	$nm=$_FILES['image']['name'];
			...
			$ip=$_SERVER['REMOTE_ADDR'];
			if($nm!=""){
				$gt=$this->db->get_where('scientific',array('id'=>$id))->row();
			...
			$add=array(
				...
				'session'=>$session,
				'biography'=>$biography,
				'abstract'=>$reaserchinterst,
				'image'=>$img
				);
			$res=$this->db->update('scientific',$add, array('id'=>$id) );
			if($res==true)
			{
				$config['file_name']  = $fname;
				$image_path = realpath(APPPATH . '../assets/aboutpage/');
