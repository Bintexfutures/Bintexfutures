...
class Model_supportjournals extends CI_Model {
...
public function add()
		...
		$img=$fname=time().'image'.rand().'.jpg';	
			...
			$res=$this->db->insert('supportjournals',$add);
...
$this->image_lib->resize();
				...
				$this->session->set_flashdata('msg','Sucessfully Add');
			}
...
public function singleView()
{
	...
	$query=$this->db->get_where('supportjournals',array('id'=>$id))->row();
	return $query;
}
...
public function update()
{
	...
	$nm=$_FILES['image']['name'];
			...
			$ip=$_SERVER['REMOTE_ADDR'];
			if($nm!=""){
				$gt=$this->db->get_where('supportjournals',array('id'=>$id))->row();
			...
			$res=$this->db->update('supportjournals',$add, array('id'=>$id) );
			if($res==true)
			{
				$config['file_name']  = $fname;
				$image_path = realpath(APPPATH . '../assets/aboutpage/');
