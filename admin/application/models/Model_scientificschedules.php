...
class Model_scientificschedules extends CI_Model {
...
public function add()
		{
			...
			$res=$this->db->insert('scientific2',$add);
...
public function singleView()
{
	...
	$query=$this->db->get_where('scientific2',array('id'=>$id))->row();
	return $query;
}
...
public function update()
{
	...
	extract($_POST);
			...
			$ip=$_SERVER['REMOTE_ADDR'];
			if(@$nm!=""){
				$gt=$this->db->get_where('scientific2',array('id'=>$id))->row();
			...
			$res=$this->db->update('scientific2',$add, array('id'=>$id) );
			if($res==true)
			{
